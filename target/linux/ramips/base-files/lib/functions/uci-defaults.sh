...
set network.lan.ipaddr='192.168.87.1'
...
set system.@button[-1].button='$button'
set system.@button[-1].action='$action'
set system.@button[-1].handler='/etc/rc.button/reboot'
set system.@button[-1].min='$min'
set system.@button[-1].max='$max'
EOF
}

ucidef_add_button_wps() {
	local button=$1
	local action=$2
	local min=$3
	local max=$4
...
set system.@button[-1].button='$button'
set system.@button[-1].action='$action'
set system.@button[-1].handler='/etc/rc.button/wps'
set system.@button[-1].min='$min'
set system.@button[-1].max='$max'
EOF
}
